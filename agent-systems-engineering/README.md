...
A central goal is to compare claims against counterevidence. Reports should explicitly identify when different organizations or papers reach different conclusions and examine whether the disagreement can be explained by differences in workload, benchmark, model, harness, scale, metrics, or environment.
...
Dated reports should prioritize genuinely new or materially revised evidence, normally presenting a selective shortlist rather than a broad news roundup. Each relevant item should capture the central claim, evidence, metrics, limitations, counterevidence, and implications for agent-system engineering.

Where useful, reports may include a contradiction matrix summarizing which sources agree or disagree on harness design, multi-agent systems, graph/loop architecture, or production operations.
